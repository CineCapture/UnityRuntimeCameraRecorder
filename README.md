...
The library captures video from Unity's normal camera render loop and captures the Unity audio mix. It delegates media transport, FFmpeg execution and output finalization to `Landoria.FFmpegMediaWriter`. On Direct3D 11 and NVIDIA systems, it can use `Landoria.D3D11NvencEncoder.dll` for native H.265 encoding.
...
Add `UnityMediaRecorder` to a Unity object, subscribe to its lifecycle events, then pass a camera, an audio listener and `RecordingSettings` to `StartRecording`. Call `StopRecording` to finish capture, preserve the high-quality MKV archive and create the MP4 file.
